...
class Graphics : public Singleton<Graphics>
{
  ...
  public:
    ...
    void BeginDraw(jc::HDevice_t* device)
    {
        if (!m_ready && device) {
            ...
            if (SUCCEEDED(hr) && factory) {
                ...
                if (SUCCEEDED(hr)) {
                    m_ready  = true;
                    m_device = device;
                    device->m_device->GetImmediateContext(&m_context);
                }
        ...
        if (m_context) {
            m_restoreState = SUCCEEDED(m_state.saveCurrentState(m_context));
        }
    }
    ...
    void DrawString(const std::string& str, float x, float y, float size, uint32_t color)
    {
        ...
        m_font->DrawString(m_context, converter.from_bytes(str).c_str(), size, x, y, color, 0);
    }
